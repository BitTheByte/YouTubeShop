...
while (1):
	try:
		...
		action = ""
		while (1):
			action = input("[*] Choose action (l = like , s = subscribe): ")
			if action == "l":
				action = "YT_LIKE"
				break
			if action == "s":
		...
		ThreadPoolSize_custom = input("[*] Choose number of threads [default = {}] [press Enter to use defaults]: ".format(ThreadPoolSize))
		...
		for file in glob.glob("*.txt"):
		    print((" |_--> " + file))
		while (1):
			combo_file = input("[*] Setect the name of your [Email:Password] Combo file: ")
			try:
		...
		while (1):
			ytfile = input("[*] Setect the name of your [YouTube] Combo file: ")
			try:
